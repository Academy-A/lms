...
from src.db.models import Setting, Student, VerifiedWorkFile
...
class BaseNotification(abc.ABC):
    ...
    def init(self) -> None:
        ...
        folder_ids = get_settings(
            session=self._session, key=self.folder_ids_prefix + self._subject_eng_name
        )
        ...
        self._regexp = get_settings(
            self._session, self.regexp_setting + self._subject_eng_name
        )
